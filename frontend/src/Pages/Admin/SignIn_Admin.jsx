...
const SignIn_Admin = ({ setUser }) => {
  ...
  const handleSignIn = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post("http://localhost:9000/Internote/signin-admin", formData);

      if (response.data.role === "ADMIN") {
        alert("Chào Admin: " + response.data.fullName);
      ...
      <form onSubmit={handleSignIn} className="max-w-md w-full mx-auto p-6 bg-white shadow-md rounded-lg border border-gray-100">
        ...
        <div className="relative z-0 w-full mb-6 group">
          <input 
            ...
            required 
          />
          <label className="absolute text-sm text-gray-500 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:text-blue-600 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">
            Full name
          </label>
        </div>
        ...
        <button type="submit" className="w-full text-white bg-blue-600 hover:bg-blue-700 font-medium rounded-lg text-sm px-5 py-3 shadow-md transition-all">
          Sign In
        </button>
      </form>
    </div>
